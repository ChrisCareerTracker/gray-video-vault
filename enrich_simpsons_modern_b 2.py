...
ERA: Modern Era Part B — Seasons 29–37 (2017–present)
Output: simpsons_modern_b.json

NOTE: Chris owns through S37E04. This script fetches all available
seasons 29–37 from TMDB and stops at whatever is available.
"""

import os, sys, json, time, re, requests

BASE = os.path.dirname(os.path.abspath(__file__))
OUT  = os.path.join(BASE, 'simpsons_modern_b.json')

TMDB_KEY      = '573382ec2121f69d6a89fce35293591a'
ANTHROPIC_KEY = os.environ.get('ANTHROPIC_API_KEY', '')
TMDB_SERIES   = 456

SEASONS = list(range(29, 38))   # 29–37

# Collection note: owned through S37E04
OWNED_THROUGH = (37, 4)   # (season, episode)
...
def is_owned(season_num, ep_num):
    owned_s, owned_e = OWNED_THROUGH
    if season_num < owned_s:
        return True
    if season_num == owned_s and ep_num <= owned_e:
        return True
    return False

def main():
    print('='*60)
    print('SIMPSONS ENRICHMENT — MODERN ERA B (S29–S37)')
    print(f'Fetching through S{OWNED_THROUGH[0]}E{OWNED_THROUGH[1]:02d} (collection cutoff)')
    print('='*60)
    ...
    for s_idx, season_num in enumerate(SEASONS):
        ...
        if not season_data:
            print(f'  Season {season_num} not found on TMDB — may not exist yet. Stopping.')
            break

        season_eps = season_data.get('episodes', [])
        if not season_eps:
            print(f'  No episodes found for season {season_num} — stopping.')
            break

        print(f'  Found {len(season_eps)} episodes on TMDB')
        ...
        for ep in season_eps:
            ...
            # Stop at collection cutoff
            if not is_owned(season_num, ep_num):
                print(f'  Reached collection cutoff at S{season_num:02d}E{ep_num:02d} — stopping.')
                break
    ...
    output = {
        'era':        'modern',
        'era_label':  'The Modern Era',
        'era_part':   'B',
        'seasons':    '29–37',
        'years':      '2017–present',
        'description': 'Springfield in the streaming age. The collection runs through Season 37.',
    ...
    print(f'\n{"="*60}')
    print(f'DONE — {len(episodes)} episodes written to simpsons_modern_b.json')
    print(f'{"="*60}')
